...
2. **Homebrew cask** — ~~submit Switchy to homebrew/homebrew-cask~~ **Investigated July 2026: not viable with the current sales model.** A cask requires a freely downloadable binary; Switchy's purchase gates the download itself (LemonSqueezy checkout), with no in-app license activation. Two additional findings from the attempt:
   - Homebrew's audit also rejects github.com download URLs from repos with <75 stars ("not notable enough") — self-hosting the DMG on mangobuns.com would solve that part.
   - **Revenue exposure worth deciding on:** the Sparkle appcast (public at mangobuns.com/switchy/appcast.xml) links the full app DMG on public GitHub releases — anyone with the URL can download the paid app today without paying. If Switchy has no license check, consider (a) accepting the obscurity risk, or (b) adding in-app license activation (LemonSqueezy has a license-key API), which would close the leak AND make Homebrew + free-trial distribution viable.
   - A ready, audit-tested cask draft is saved at `docs/homebrew-cask-switchy.rb.draft` for if/when licensing changes.
