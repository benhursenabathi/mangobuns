...
2. **Homebrew cask** — **viable as of v1.1.4 build 70 (2026-07-10)** (trial + license activation = the free-download/paid-activation model Homebrew supports). Full execution plan: **`docs/homebrew-cask-plan.md`** (why it helps, how users find the one-liner, versioned-DMG requirement, validation steps, PR flow, self-hosted-tap fallback). The switchy-build skill already refreshes the trial DMG each release.
