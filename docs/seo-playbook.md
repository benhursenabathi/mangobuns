...
2. **Homebrew cask** — submit Switchy to `homebrew/homebrew-cask` so devs can `brew install --cask switchy`. You already have everything required: stable versioned download URLs + Sparkle appcast. This is discovery among exactly the multi-Mac power-user demographic, plus a backlink from one of GitHub's most-starred repos.
